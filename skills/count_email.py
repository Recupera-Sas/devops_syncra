...
def function_complete_EMAIL(input_folder, output_folder, partitions, Widget_Process):
    ...
    #lsit = [consolidated_df.columns]
    #print(lsit)

    if consolidated_df is not None:

        consolidated_df = consolidated_df.withColumn("Cuenta_Next", col("Cuenta"))
        selected_columns = [
            "Cuenta_Next", "Cuenta", 
            "Edad_Mora"]
        ...
        delimiter = ";"
        Type_Proccess = "Consolidado EMAIL"
...
def Function_Modify(RDD):
    ...
    Data_Frame = RDD 
    Data_Frame = Data_Frame.filter(col("Cuenta") != "0")
    ...
    Data_Frame = Data_Frame.withColumn("Recurso", lit("EMAIL"))
