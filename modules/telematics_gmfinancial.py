...
def function_complete_telematics(path, output_directory, partitions, process_resource):
    ...
    print(f"Processing Telematics GM Financial with resource: {process_resource}")
...
def Renamed_Column(Data_Frame):
    ...
    Data_Frame = Data_Frame.withColumnRenamed("id_usuario", "ID_Crediveci")
...
def Save_Data_Frame (Data_Frame, Directory_to_Save, partitions, resource):
    ...
    Type_File = f"BD GM Financial {resource}"
    delimiter = ";"
...
def Phone_Data(Data_):
    ...
    columns_to_stack_min = ["numeromarcado", "telefono_casa", "telefono_oficina", "celular", "celular_codeudor", "tel_casa_codeudor", "tel_oficina_codeudor"]
    all_columns_to_stack = columns_to_stack_celular + columns_to_stack_min
...
def Email_Data(Data_):
    ...
    column_new = ["correo_codeudor", "correo_electronico"]
    columns_to_drop = column_new #+ column_new
...
def conversion_process (Data_Frame, output_directory, partitions, Contacts_Min):
    ...
    Data_ = Data_.withColumn("Rango", \
            when((col("deuda_total") <= 20000), lit("1 Menos a 20 mil")) \
                .when((col("deuda_total") <= 50000), lit("2 Entre 20 a 50 mil")) \
                .when((col("deuda_total") <= 100000), lit("3 Entre 50 a 100 mil")) \
                .when((col("deuda_total") <= 150000), lit("4 Entre 100 a 150 mil")) \
                .when((col("deuda_total") <= 200000), lit("5 Entre 150 mil a 200 mil")) \
                .when((col("deuda_total") <= 300000), lit("6 Entre 200 mil a 300 mil")) \
                .when((col("deuda_total") <= 500000), lit("7 Entre 300 mil a 500 mil")) \
                .when((col("deuda_total") <= 1000000), lit("8 Entre 500 mil a 1 Millon")) \
                .when((col("deuda_total") <= 2000000), lit("9 Entre 1 a 2 millones")) \
